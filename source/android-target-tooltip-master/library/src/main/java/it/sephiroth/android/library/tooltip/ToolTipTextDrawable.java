...
class TooltipTextDrawable extends Drawable {
    ...
    private float arrowLengthWeight = 1;

    public float getArrowLengthWeight() {
        return arrowLengthWeight;
    }

    public void setArrowLengthWeight(float arrowLengthWeight) {
        this.arrowLengthWeight = arrowLengthWeight;
    }


    private TooltipManager.Gravity gravity;

    public TooltipManager.Gravity getGravity() {
        return gravity;
    }

    public void setGravity(TooltipManager.Gravity gravity) {
	...
	private void calculatePath(Rect outBounds) {
        ...
        float realArrowLengthWeight = arrowLengthWeight *2;

        if (gravity == TooltipManager.Gravity.BOTTOM) {

            top = outBounds.top + (int)(padding *realArrowLengthWeight);

        } else if (gravity == TooltipManager.Gravity.TOP) {

            bottom = outBounds.bottom - (int)(padding *realArrowLengthWeight);

        } else if (gravity == TooltipManager.Gravity.LEFT) {

            right = outBounds.right - (int)(padding *realArrowLengthWeight);

        } else if (gravity == TooltipManager.Gravity.RIGHT) {

            left = outBounds.left + (int)(padding *realArrowLengthWeight);
